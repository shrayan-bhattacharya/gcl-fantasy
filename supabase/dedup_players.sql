...
BEGIN
  ...
  -- Purge any remaining rows without cricapi_player_id (ESPN-era orphans with no CricAPI match)
  DELETE FROM public.player_match_stats
    WHERE player_id IN (SELECT id FROM public.ipl_players WHERE cricapi_player_id IS NULL);
